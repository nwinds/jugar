...
class Hand(object):
    ...
    def __str__(self):
        ...
        for letter in sorted(self.hand.keys()):
            #print('types: output:%s, letter:%s, hand[letter]:%s' % \
            #(str(type(output)), str(type(letter)), str(type(self.hand[letter]))) )
    ...
    def update(self, word):
        ...
        """
        currHand = self.hand.copy()
        for i in range(len(word)):
            char = word[i]
            if currHand.get(char, 0) > 0:
                currHand[char] = currHand.get(char, 0) - 1
            else:
                return False
        self.hand = currHand
        return True
    def updateReferenceAnswer(self, word):
        """
        Does not assume that self.hand has all the letters in word.
...
myHand = Hand(12)
myHand.setDummyHand('eyeyeytwywje')
assert myHand.update('chayote') == False
print myHand
#eeeejtwwyyyy

myHand = Hand(8)
myHand.setDummyHand('ttaqiuxz')
assert myHand.update('tea') == False
print myHand
#aiqttuxz

myHand = Hand(9)
myHand.setDummyHand('booblsxpe')
assert myHand.update('boar') == False
print myHand
#bbeloopsx

myHand = Hand(11)
myHand.setDummyHand('otoaccbfesx')
assert myHand.update('carrot') == False
print myHand
#abccefoostx
