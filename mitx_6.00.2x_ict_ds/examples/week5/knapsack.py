...
class Item(object):
    ...
    def __str__(self):
        ...
        return result 
    def __repr__(self):
        """ my impl for psets debugging"""
        return '<name=%s, value=%.2f, weight=%.2f>' % (self.name, self.value, self.weight)
...
def dToB(n, numDigits):
    ...
    my explanation:
        mapping digits representation to real object rep:
            Y (digits) -> X (real Items)
    """              
...
def chooseBest(pset, constraint, getVal, getWeight):
    bestVal = 0.0 # initial best value
    bestSet = None # initial best set
    for Items in pset:
        ItemsVal = 0.0
        ItemsWeight = 0.0
        """my review: Bruce force approach: add all v and w in provided subsets, 
        then compare and decide whether update the best case or not"""
        for item in Items:
